...
EMPLOYEES_CSV_HEADERS = ['name', 'state', 'dept_code']
...
with open('generator/employees.csv', 'w') as employees_csv:
    employees_writer = csv.DictWriter(employees_csv, fieldnames=EMPLOYEES_CSV_HEADERS)
    employees_writer.writeheader()

    for i in range(NUM_EMPLOYEES):
        employees_writer.writerow(dict(
            name=fake.user_name(),
            state=fake.sentance(),
            dept_code=1

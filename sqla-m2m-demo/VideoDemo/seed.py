...
free = Employee(name="freelancer mcgee")


db.session.add_all([river, summer, joaquin, octavia, larry, kurt, rain, free])

db.session.commit()


p1 = Project(proj_code="boat", proj_name="Build massive boat")
db.session.add(p1)
db.session.commit()

r1 = EmployeeProject(proj_code=p1.proj_code, emp_id=free.id)
db.session.add(r1)
db.session.commit()

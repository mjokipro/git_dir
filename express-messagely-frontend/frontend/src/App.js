...
// export const token = "messagely-token"
...
function App() {
  ...
  const [_token, set_Token] = useLocalStorage(MessagelyApi._token)

  
  useEffect(
    function loadingUser(){
     console.debug("user=", user, "noMoreRecs=", noMoreRecs, "_token=", _token)
      async function getUser(){
        if (_token) {
          try {
              let {username} = jwt.decode(_token)
              MessagelyApi._token = _token
  ...
  async function register(signupData){
    try {
        let _token = await MessagelyApi.register(signupData)
        set_Token(_token)
        return {success: true}
    } catch(errors) {
